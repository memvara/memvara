...
memory alone, instead of from everything else that happens to share its words. A
label narrows the answer and does not rank it, so read without one when the answer
could be anywhere, and when a narrowed read comes back empty, say that nothing
under that label matched rather than that nothing is stored.
