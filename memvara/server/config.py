...
class ServerConfig:
    ...
    #:
    #: That default is why this exists. A self-hosted model generating at about 5 tokens a
    #: second needs longer than 600 seconds for a turn of a few thousand characters, and a
    #: cancelled call is a turn that was not extracted, not one stored with fewer claims.
    #: Measured on a production worker on 2026-09-11: turns of around 10,000 characters
    #: were cancelled at the 600-second mark after generating more than 3,200 tokens, every
    #: pass, for five days, because nothing recorded the failure and the same turn was
    #: picked again.
    #:
    #: **Which value is right is a property of the model and where it runs.** The same
    #: worker against a 27B on a GPU over a LAN answers that turn in 13.7 seconds, where a
    #: long timeout stops being a ceiling and becomes a long wait on a connection that has
    #: already died. Raising this lets a slow turn finish; it does not make a turn that
    #: cannot finish safe to retry, and it does not bound a runaway — that is
...
def unknown_features(names: Iterable[str]) -> str | None:
    ...
    "'profle' (did you mean 'profile'?) is not a feature. The features are index_command, research_agent, project_scope, status_line, recall_mark, profile, forget_matching, end_reason, links, documents, retrieval_chunks and extraction_chunks."
...
def _timeout(raw: str | None) -> float | None:
    ...
    A decimal is accepted because a timeout is a duration rather than a count, so `900.5`
    is a sensible thing to write. Refused rather than clamped for the reason its
    neighbours are: a value that fell back to the default would leave an operator
