    ...
    Tool(
        ...
        properties={
            ...
            "role": {
                ...
                "default": "user",
                "description": (
                    "Who said it. This decides what gets extracted from the text, not "
                    "only who it is attributed to, and 'user' is the only value "
                    "extraction reads. On a user turn a deterministic matcher takes a "
                    "name, a home, an employer, a job title, a language or an allergy "
                    "stated in the first person, from anywhere in the text, and writes "
                    "it as a fact about the person at high confidence — enough to "
                    "supersede a value they stated outright. Where a model is "
                    "configured a second pass reads the same turn for anything else in "
                    "the vocabulary. The matcher searches rather than anchors and "
                    "removes quotation marks before it looks, so such a sentence "
                    "sitting inside a log, a document or a docstring cannot be told "
                    "from one the person wrote. Send 'system' for a transcript, a log, "
                    "a pasted document, or anything else composed rather than spoken: "
                    "the text is stored whole and the receipt still returns its turn "
                    "ids, and no fact is extracted from it. That is narrower than "
                    "inert — a turn closely repeating one already held still "
                    "strengthens the claim it matches and is recorded as a source for "
                    "it, at any role — so it lowers the risk rather than removing it. "
                    "'assistant' marks something you said; extraction declines it "
                    "exactly as it declines 'system', so choosing between those two "
                    "is about attribution and not about how far either is trusted."
                ),
            },
        },
        required=("text",),
