"""CrewAI: the unified-memory `StorageBackend`, implemented on memvara.
...
text — CrewAI embeds the query with its own model before the backend is reached. Memvara
...
`text-embedding-3-large` is not even a point in memvara's space. Two dishonest ways out
were available and both were rejected — store CrewAI's vectors and degrade to cosine
top-k (memvara becomes a numpy matmul with extra steps), or re-embed text this interface
...
slots, and both stay live — measured against the real package, not argued. Memvara's
...
they hand memvara *raw turns* and extraction runs; this one is handed CrewAI's already-
extracted output. Getting it back means extracting triples from the sentences, which
costs a model call per record — the same phase-2 trade `memvara.compat.mem0_import`
...
* **Scope trees point in opposite directions.** CrewAI's `/company/team/user` is an
  aggregation tree: a search at `/company` should see everything beneath it. Memvara's
  ...
  adapter does not pretend — the memvara scope is fixed at construction (and is what
  isolates two `MemvaraStorage`s from each other), while CrewAI's path is stored per
...
_NO_QUERY_TEXT = (
    ...
    "and memvara does not retrieve from a vector — it fuses BM25 with vector search and "
    "rescores by a per-predicate half-life, all of it keyed on the query string. A "
    "vector from another model is not even a point in memvara's space.\n\n"
    "So this backend embeds through memvara's own embedder and remembers what it "
    "embedded, which is what makes the query text recoverable here. Wire it up:\n"
    "    storage = MemvaraStorage(mem, user='alice')\n"
    "    Memory(storage=storage, embedder=storage.embedder)\n\n"
    "The vector just received has {got} dimensions and memvara's embedder produces "
    ...
    "MemvaraStorage(query_cache=...)."
)

_NO_METADATA_FILTER = (
    "memvara has no metadata index, so metadata_filter= would have to be applied after "
    ...
    "meta['crewai']['metadata'] yourself through Memvara.search()."
)
...
_RETIRED_NOT_ERASED = (
    ...
    "and its embedding remain on disk — still reachable through Memvara.history() and "
    "Memvara.search(as_of=...).\n\n"
...
class CrewAICompatError(IntegrationError):
    """A CrewAI storage call with no honest translation onto memvara."""
...
class MemvaraStorage:
    """Memvara behind CrewAI's `StorageBackend` protocol.

    Structural, not nominal: `StorageBackend` is a `Protocol`, so this class satisfies
    it by having the methods and never imports it. Construct one per memvara scope — that
    ...
    def __init__(self, memory: Any, *, tenant: str | None = None, user: str | None = None,
        ...
        if on_delete not in _ON_DELETE:
            raise ValueError(
                ...
                "memvara.integrations.crewai.CrewAIDeletionWarning for what each one does"
            )
    ...
    def embedder(self) -> Any:
        ...
        *memvara's* embedder, so every vector CrewAI holds is a point in the space
        memvara's index actually uses — and it remembers the text behind each vector, so
        `search()` can recover the query CrewAI declined to pass. See `_NO_QUERY_TEXT`.
        """
        return self._embed

    def _embed(self, texts: Sequence[str]) -> list[list[float]]:
        """Encode with memvara's embedder, and remember what each vector was.
    ...
    def _blob(record: Any) -> dict[str, Any]:
        """The CrewAI fields memvara's data model has no column for.
    ...
    def _to_record(self, claim: Claim) -> Any:
        ...
        return self._crewai().MemoryRecord(
            ...
            # Memvara does not track reads, so the honest answer is the last write. A
    ...
    def save(self, records: Sequence[Any]) -> None:
        ...
        Backdated to `record.created_at` on both memvara axes, so `search(as_of=…)` over
    ...
    def get_record(self, record_id: str) -> Any | None:
        ...
        `Memvara.history()` still reaches every version, which is the recovery path a
    ...
    def reset(self, scope_prefix: str | None = None) -> None:
        ...
        With no `scope_prefix` this is `Memvara.purge()` on the bound scope — claims,
    ...
    #
    # `asyncio.to_thread`, not an async rewrite. Memvara is synchronous and SQLite has no
    # async driver worth the name (see `memvara.aio`); what these buy is an event loop
    ...
    def __repr__(self) -> str:
        return (f"<MemvaraStorage {self.scope.key()} on_delete={self.on_delete} "
                f"of {self.memory!r}>")


__all__ = ["MemvaraStorage", "CrewAICompatError", "CrewAIDeletionWarning",
