...
- **`OpenAILLM`** — the `engram[openai]` extra has been declared since the first commit
...
- **Model-backend validation is now shared** (`engram/llm/_shape.py`). It was private to
...
- **Entity resolution** (`engram/entities.py`). `entity_key()` is a pure fold applied
...
- **Per-claim erasure** — `store.erase_claim`, `Engram.erase(claim_id, sources=False)`.
  Distinct from `delete()`, which retires.
- **Telemetry** (`engram/telemetry.py`). A `Recorder` protocol, default `None` rather than
...
- **`AsyncEngram`** (`engram/aio.py`) — each public method over `asyncio.to_thread`.
- **`Engram.supersede`**, **`remember(sources=, text=, extractor=)`**, **`get`**,
  **`count`**, **`reset`**, **`scope`**.
...
- `Engram.add`'s outer batch narrowed to the indexing half — it had been holding the write
  lock across the whole call and defeating the hoist end to end (2 → 1,030 reads during a
  1 s extraction).

### Fixed

- The mem0 compat layer leaked engram's internal bookkeeping keys as caller metadata.
...
- **MCP server** (`python -m engram.server`) — eight tools over JSON-RPC 2.0 stdio, no SDK
  dependency. `consolidate`, `purge`, `reset` and `erase` are deliberately absent.
- **mem0 compatibility** (`engram.compat.Memory`) and the **`history.db` importer**, which
