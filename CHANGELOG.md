...
- **`examples/`, and a test suite that runs it.** Three programs a developer can run
  ...
  from the library the way a snippet in a document can. The demo's transcript is a golden
...
- **A documentation tree with a route through it.** `docs/README.md` is the index, and
  fifteen new pages sit under `getting-started/`, `concepts/`, `guides/`, `integrations/`
  and `reference/` — including the two explanations the repository did not have anywhere:
  ...
  rather than which one wins. `docs/FAQ.md` answers ten questions against the
