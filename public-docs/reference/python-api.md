...
|---|---|---|
| `forget(subject, predicate, *, at=None, close="retired")` | `list[Claim]` | Close every current value in this slot, and any value stored to begin later. A value that has already ended is left as it is. `close="retired"` (default) means the record was wrong; `close="ended"` means the world changed. |
