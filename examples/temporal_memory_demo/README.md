...
**No GIF or video is checked in**, and that is deliberate rather than an omission: a
GIF is a build product of `demo.py` and one of the recipes below, and a binary that has
to be regenerated by hand every time the script changes is a binary that goes stale
without going red. Generate one when you need one.

Three routes, in the order to try them. The first two want tooling; the third wants only
Pillow and works in a container with no terminal at all.
