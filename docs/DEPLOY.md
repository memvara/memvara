...
JSON-RPC 2.0 over stdio, twenty-two tools, no SDK dependency. It refuses to start without
...
| `MEMVARA_EXTRACT_GUIDANCE` | Path to a TOML file of guidance for this project's extraction, added to the extraction instructions under a fixed heading rather than replacing them: `context` (a description of the project, at most 1,500 characters), `include` (things worth extracting) and `exclude` (things to leave out), each list at most 20 rules of at most 200 characters. For example `context = "A payments service."` and `exclude = ["stack traces"]`. Applies to `MEMVARA_LLM=anthropic` and `openai`, and to a replacement prompt from `MEMVARA_LLM_EXTRACT_SYSTEM` too. Read and checked at startup: a missing file, invalid TOML, an unknown key or a rule over the limit is refused, and so is the variable with `MEMVARA_LLM=none`, where no model would read it. Needs Python 3.11 or later, where Python's TOML reader arrives; on 3.10 the server refuses to start with it set. Refused under `MEMVARA_MODE=cloud`. `MEMVARA_FEATURE_EXTRACTION_GUIDANCE=0` switches it off. |
...
| `MEMVARA_CLOSED_VOCABULARY` | `1` makes the write pipeline refuse a model-proposed claim whose predicate is not one the registry knows (a builtin, a declared alias, or a pack loaded through `MEMVARA_PREDICATES`), and count it on the receipt's `unregistered`. No model call is spent learning the predicate. Off by default. Nothing a `memory_remember` asserts is affected. Set it on a deployment whose vocabulary is deliberate: an undeclared predicate is multi-valued forever and supersedes nothing, so it only adds noise to every recall. |
...
| `MEMVARA_NAT64_PREFIXES` | Your own NAT64 prefixes, separated by commas, such as `2a01:4f8:c0c:64::/96`. When a document is fetched by URL and its host resolves to an address inside one of these prefixes, the IPv4 address inside it is checked instead, so a private IPv4 host reached through your NAT64 gateway is refused. The well-known prefix `64:ff9b::/96` and the local-use prefix `64:ff9b:1::/48` are always checked and need not be listed. Each prefix must be 32, 40, 48, 56, 64 or 96 bits long, as RFC 6052 defines, or the server refuses to start. Set it if this server runs on an IPv6-only network with a NAT64 gateway on any other prefix; an address translated through a prefix that is not listed cannot be recognised. No tool in this server fetches URLs yet; `ServerConfig.url_fetcher()` is the fetcher the document tools will use. |
...
| `memory.db.lock` | an empty file each open store holds a lock on, so that clearing the vectors can tell whether anything else has the store open. A store opening also takes it for a moment, so that only one process at a time creates or upgrades the database, which means every process that opens the store must be able to write it. `memory.db.lock-journal` appears beside it while that happens | nothing once every process has closed the store: the next open recreates it. Deleted while a store is open, it stops `reembed()` from seeing that store |
