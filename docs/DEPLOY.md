...
| `MEMVARA_FEATURE_<NAME>` | `0` switches one feature off and `1` switches it on. Every feature is on by default except the ones marked (off by default) here, which are off until their variable says `1`. `MEMVARA_FEATURE_PROJECT_SCOPE=0` stops the project being worked out from the working directory (an explicit `MEMVARA_PROJECT` still applies), `MEMVARA_FEATURE_PROFILE=0` hides `memory_profile`, `MEMVARA_FEATURE_FORGET_MATCHING=0` hides `memory_end_matching` and `memory_forget_matching`, `MEMVARA_FEATURE_LINKS=0` hides `memory_link`, and `MEMVARA_FEATURE_END_REASON=0` removes the `reason` and `until_reason` arguments from every tool that has them (reasons already stored are still shown). `MEMVARA_FEATURE_EXTRACTION_CHUNKS=1` (off by default) extracts a turn over 6,000 characters in pieces, one model call per piece; it is off because its release bar is not met (`docs/ROADMAP.md`, the "Reversed" list), and it matters only when `MEMVARA_LLM` names a model. `INDEX_COMMAND`, `RESEARCH_AGENT`, `STATUS_LINE` and `RECALL_MARK` are accepted as well and change nothing in this server; they are for the plugin. `INGEST_URLS` and `INGEST_MEDIA` are accepted too: they switch off fetching a URL and reading images, audio and video when a document is added, and no tool in this server adds documents yet. An unknown name, or a value that is not a boolean, is refused at startup. `memory_stats` lists the features that are off. |
