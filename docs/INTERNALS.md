...
7. **A filter and a limit may not live in different layers.** Whatever narrows rows has to
   ...
   not a comprehension in the facade: `search` over-fetches `k * candidate_multiplier`,
   never fewer than `candidate_floor`, and ranks those, so a state filter applied
   afterwards finds a retired claim only when it happens to land inside that window —
   twelve live rows against `k=1` with the floor off is a window of five, and the audit
   ...
   > `states=` half is measured too: twelve live rows against `k=1`, floor off, is a
   > window of five.
...
- run vector and lexical retrieval over `k * candidate_multiplier` candidates each, and
  ...
  the best-scoring claim before fusion saw it (#155). The floor changes nothing at
  `k >= 10`, and `0` disables it;
