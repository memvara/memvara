...
| `memory_remember` | Record one exact fact as a triple, skipping extraction entirely. With `replaces`, it ends one named fact in the same write and records why. With `expires_at`, the fact stops being returned the moment that instant passes and the store **erases** it within the hour, and `expire_reason` says why |
