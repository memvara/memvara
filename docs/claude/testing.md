...
- **What they found.** #280: a damaged embedder record let a same-width embedder change go unnoticed. It is fixed: the open now warns that it cannot tell which embedder wrote the vectors and writes the record again, and the test that pinned the bug checks both. #281: two processes opening a new store at once can make one of them fail at startup. It is pinned as a strict expected failure.
