...
- **The tests catch the breaks they are for.** They were run against seven deliberately broken copies of the migrations, each in a scratch directory, and each break failed the test aimed at it, for exactly the stores it affects: no `optimize` in `_migrate_to_v7`, no re-keying in `_migrate_to_v12`, no `_migrate_to_v15`, a version stamped one too low, a backfill of `object_kind`, a write on every open that migrates nothing, and turns moved into a project.
