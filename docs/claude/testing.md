...
- **What every pull request checks.** Each store is unpacked into the test's temporary directory, so no test can change a committed file. Opened with this code, it must open without a warning, reach today's schema version, dump exactly as `golden.json` says, pass the integrity checks, and not change on a second open. Every claim must read back with the same subject, predicate, object and clocks. Every live claim and every turn must be found by search in its own scope, and no claim may be found by a reader in another user's or tenant's scope. Every document and every erasure record must still be there. Restating a stored value must reinforce the stored claim, which shows that versions 6, 12 and 16 re-keyed it correctly. A claim an old release erased must stay unreadable, including the words a release before version 7 left in the text index.
