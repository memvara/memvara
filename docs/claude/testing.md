...
- **What the folder does not compare yet.** Two differences it found are documented nowhere, so they were reported to the maintainer, to be filed and pinned as strict expected failures, rather than asserted here. A write receipt read through a hosted client has empty `accumulated`, `disputed`, `collapsed` and `retyped` lists where the local receipt reports what the write did, so in cloud mode `memory_remember` leaves out the four notes built from them; until the issue is filed, both programs leave out the writes that produce those outcomes. And the hosted clients' `end()` has no local twin, while docs/API.md says `service()` is the only hosted method without one.
