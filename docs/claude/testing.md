...
- **Giving the hooks a store.** Pass `server_env={"MEMVARA_DB": ..., "MEMVARA_USER": ...}` and the runner writes a memvara server block where and how the host itself keeps its MCP servers, which is where a user who runs a local store configures it (`CLIENT_CONFIGS`): `mcpServers` in `~/.claude.json` for Claude Code, a TOML `[mcp_servers.memvara]` table in `~/.codex/config.toml` for Codex, `mcpServers` in `~/.copilot/mcp-config.json` for Copilot, `mcpServers` in `~/.cursor/mcp.json` for Cursor, and `mcp` in `~/.config/opencode/opencode.json` for OpenCode. The hooks look for the store in the files the host record lists, so a test can check that the two agree. Without a store, the hooks report "not configured". `env={"MEMVARA_DB": ...}` names a store another way, through the hook process's own environment, which the hooks read on every host and which wins over any client config.
...
- **The recall daemon is off unless a test asks for it.** `daemon=True` lets the recall hook start its background daemon, which `child_env` forbids because the daemon outlives the hook. `wait_for_daemon()` returns its socket and its pid once it accepts a connection. The pid comes from the kernel, through the socket (`socket_peer_pid`), because every daemon runs the same command. The runner also puts a small `sitecustomize` module first on the hook's `PYTHONPATH`, which Python runs when any process inheriting that path starts, so every daemon the hooks start records its pid: `daemon_pids()` lists them, which lets a test count the daemons one hook started. `close()` kills each recorded daemon that still runs this checkout's `daemon.py`, and each one listening under the runner's home, so a daemon that no socket path leads to any more is stopped too. The socket lives under the runner's home, and macOS refuses a unix socket path of 104 bytes or more, so a home for a daemon test needs a short path, which `short_dir` makes. It makes the home in the system's temporary directory, and makes it again in /tmp when the daemon's socket would not fit under it: `daemon_socket_path` gives the socket's path and `MAX_SOCKET_PATH` the limit. It measures the resolved path, because `child_env` resolves a home before the hooks see it, and a temporary directory is often reached through a symbolic link, as /tmp is on macOS. Windows has no unix sockets, so these tests skip there with the reason `NO_UNIX_SOCKETS`. `HookRunner` refuses `daemon=True` with the same words, so a skip made from that refusal matches the skip ledger's rule.
- **A time limit can be shrunk instead of waited out.** `patches={"lib.extract.TIMEOUT_SEC": 1.0}` sets that module attribute inside the hook process before the hook runs, through a small launcher that binds the host first, as `run.py` does, and then calls `run.py`'s own `main`. A patch reaches code that reads the module attribute when it runs; a module that copied the value with `from x import NAME` keeps its own copy, and so does a default argument. `lib.fast.REWRITE_WAIT_SEC` is such a value: a patch of it changes nothing, and a test that wants a shorter rewrite wait patches `recall.REWRITE_WAIT_SEC`, the copy recall.py reads. A patch that names an attribute the hooks do not have is refused with `ValueError`, so a renamed limit cannot leave a test waiting out the real one. Patches cannot reach a capture that a host hands to a child, because `run.py` starts that child afresh, so they are refused there too.
...
- **What they found.** #280: a damaged embedder record lets a same-width embedder change go unnoticed. It is pinned as a strict expected failure. #281: two processes opening a new store at once could make one of them fail at startup. It is fixed: one store at a time now runs the schema and the migrations, and the switch to WAL mode is retried within the busy timeout. The tests that pinned it pass as ordinary tests, including the nightly one that opens one new store from three processes at once, 60 times. That nightly test also showed that the retry alone was not enough: with only the retry it failed in 3 of 10 runs, and a probe traced such failures to a migration that failed with "vtable constructor failed".
...
The runner writes the `seed` through the library first. The seed stands for memory from conversations before this one. Then every session starts its own server process on the same store file, the way a client starts one per conversation, and plays its turns in order. A turn holds the user's words and a `script`: the steps a careful agent would take for that turn. When a session ends, its server is closed; when a step fails, the server is killed. Either way, every hook runner the session made is closed, so no daemon or capture child its hooks started outlives the session.
...
- `HangingClis(directory)` writes a `claude` and a `codex` that never answer: each sleeps until it is killed. It has the same `path()`, and the hook tests use it to check what capture does when its extractor hangs.
- The fakes are POSIX shell scripts. On Windows a program that another starts without a shell is found on `PATH` only as an `.exe`, so both classes refuse to run there, and the tests that need them skip with the reason `NO_FAKES`. The `clis` fixture in `tests/adversarial/conftest.py` gives a test a fresh `FakeClis`, or that skip.
...
`tests/adversarial/hooks/` checks the plugin's hooks the way each of the five hosts uses them: Claude Code, Codex, GitHub Copilot CLI, Cursor and OpenCode. Every test runs the real `plugin/hooks/run.py <hook> --host <host>` through `HookRunner`, with the payload that host sends. The design's hook row is the specification, and the plan is `docs/superpowers/plans/2026-09-26-adversarial-hooks.md`.

Run the fast tier with `pytest tests/adversarial/hooks`, which took 21 seconds on a laptop that other test runs were keeping busy. Run the nightly tier with `pytest tests/adversarial/hooks/nightly --tier nightly`, which took about 110 seconds; about 70 of those are the time limits that two known bugs make their pins wait out (see "Known bugs" below).

- **What each host reads comes from its record.** `support.py` restates by hand the measurements that each record in `plugin/hooks/hosts/` holds: the event the host fires for each hook, whether it reads a reply nested in `hookSpecificOutput`, which key carries the status line and which the context, and how an approval is spelled. A record or a renderer that drifts from those measurements fails here, instead of shipping a reply the client ignores. Nobody has measured the tool names Cursor and OpenCode give the approve hook, so those tests use the first separator each record lists. The restatement is deliberate, because a test that read the records would agree with any change to them. So each table is also compared with the records directly: `HOSTS`, `EVENTS`, `SHAPES` and `DETACHES` at the end of `test_adv_hook_envelopes.py`, `TOOL_NAMES` in `test_adv_hook_approve.py`, and `LIMITS` in `test_adv_hook_timeouts.py`. When a record changes, the failure names the table that has gone stale.
- **Many runs at once.** A module that needs many hook runs makes them once, side by side in threads, in a module-scoped fixture (`support.run_all`), and each test reads its own result. Every run has a home of its own, so no run sees another's logs or state. When a fast module and a nightly one check the same property, the body of the check is one function in `support.py`, and the two modules differ only in the hosts and payloads they pass it.
- **Envelopes** (`test_adv_hook_envelopes.py`). Every hook on every host, against a store that holds one memory. Session start and recall put it under the host's context key; approve allows a read-only tool, named the way that host names it, and says nothing about a tool that writes; capture prints nothing on any host, and stores the fact its turn states; and `run.py` skips recall on Cursor, which never fires that event, with a line in `hooks.log`.
- **Where the hooks find the store** (`test_adv_hook_client_config.py`). On Claude Code and on Copilot, session start finds the store that the host's own MCP config names, and a `MEMVARA_DB` in the hook's own environment wins over it. Codex, Cursor and OpenCode keep their MCP servers in a file or a shape the hooks do not read: Codex in TOML, Cursor in `~/.cursor/mcp.json`, and OpenCode under `mcp`. On those hosts a store can be named only through the hook's own environment, which is how the tests name it on Codex. This is a known bug, B59, and the test that finds the store on Claude Code and Copilot is pinned as an expected failure on the other three hosts.
- **Hostile payloads** (`test_adv_hook_hostile.py`). Every hook gets `{}`, empty stdin, text that is not JSON, a JSON list, invalid UTF-8, a prompt that is a lone surrogate, fields of the wrong type, unknown fields, 8 MB, and nesting 100,000 levels deep, against a store that holds one memory. Each must exit 0, write nothing to stderr, print nothing or one JSON object, and finish within its host's limit. Four payloads are not a JSON object once read: empty stdin, text that is not JSON, a JSON list, and invalid UTF-8 read strictly. Each must be answered exactly as `{}` is, with no crash of the hook's body logged by `run.py`, because `read_event` promises that anything unreadable becomes an empty event; the nightly tier adds a JSON string, `null` and a number to those. No payload may start an extraction. Invalid UTF-8 is sent twice, once with `PYTHONIOENCODING=utf-8:strict` and once with `utf-8:surrogateescape`, because on Linux and macOS a child's stdin decodes it one of those two ways depending on the locale, and the result must not depend on the machine. Windows decodes a pipe with the locale's code page, which these tests do not cover. The fast tier covers Claude Code and Cursor, which between them take every path through `run.py`; the nightly tier covers all five hosts and adds three JSON values that are not objects.
- **Outcomes** (`test_adv_hook_outcomes.py`). Session start and recall can end five ways: nothing is configured; a local store cannot open (a file that is not a SQLite database); a hosted store cannot be reached (`FakeHostedMcp` refusing the handshake); the store answers with nothing that matches (for session start, an empty store); or it answers with memories. The tests compare two outcomes by what a person or a log reader can see of each run, the reply and the new log lines, and check that injected memories are told apart from every other outcome, that a store that could not be reached is told apart from one that answered, that recall logs `failed reason=` when it could not ask, and that each outcome has its own words on Claude Code, the one host with a status line. The fast tier checks Claude Code and Copilot, and the nightly tier all five hosts.
- **Deduplication** (`test_adv_hook_dedup.py`). Recall injects a memory once per session and again in a new session, on a host with a status line and on one without. Capture mines a turn once however often `Stop` fires over it, and mines the next turn when the transcript grows. Each test was seen to fail with the hook's own record, `recalled/<session>.json` or `capture-state.json`, deleted between runs.
- **The recall daemon** (`test_adv_hook_daemon.py`, POSIX only). The first recall starts one daemon, whose socket is 0600 inside a 0700 directory, even when that directory already existed with looser modes. A later recall is answered by that daemon rather than starting another: its import trace (`PYTHONPROFILEIMPORTTIME=1`) shows no `memvara` import, the socket's pid is unchanged, and `daemon_pids()` still holds one pid. The control, a recall with no daemon allowed, does import `memvara`. The later recall is in the same session as the first, because the first prompt of a session refreshes the standing preferences in the hook's own process. The nightly tier kills a daemon and checks that the next one takes over the socket file it left.
- **The generated registration** (`test_adv_hook_registration.py`). `plugin/hooks/tools/generate.py <host>` writes the `hooks.json` a client reads. This repository commits none, because each plugin repository generates its own and compares it there, so these tests check what can be checked here. For each host with shell hooks, generating twice, in a copy of `plugin/hooks`, writes the same bytes as `registration()` builds in process; the registration names every event the host record has and nothing else, each command runs `run.py` for its hook and host from that host's plugin-root variables, each timeout is the host's limit, only Claude Code's capture runs in the background, only Codex declares a context limit, and approve uses the record's matcher. OpenCode, whose hooks are JavaScript, is refused, and the file already there is left as it was.
- **Time limits** (`test_adv_hook_timeouts.py`). Each host kills a hook at its limit: session start 20 seconds, recall 10, approve 5, capture 180 on Claude Code and 120 elsewhere. The tests never wait a real limit out in the fast tier. They check that every host record declares the contract's limits; that recall stops optional work at 7.5 seconds, between the 5-second rewrite wait and its limit; that, with that budget patched to 0, recall skips its standing refresh and its wider second read, logs both, and still answers; that each host's capture limit covers every extraction its capture can run; that capture on Claude Code, with both extraction limits patched to a second and a `claude` that never answers (`HangingClis`), exits 0 in time, logs both timeouts, and puts the capture alert on the next prompt's status line; and that on Codex a capture whose extractor hangs returns within 3 seconds with its child still running. That check reads the child with `process_alive`, because a child that has ended but has not been reaped still answers signal 0, and a test hands the check such a child to show that it then fails. Approve starts no program and reads no store, so its limit is covered by every approve run finishing inside 5 seconds. The nightly tier waits recall's real 7.5-second budget out against a hosted endpoint whose standing calls are slow, and runs the Codex check on Copilot and Cursor too.
- **The approve list** (`test_adv_hook_approve.py`). The approve hook's list of tools it lets run without asking must be the tools the real server marks `readOnlyHint` in its `tools/list`, read over stdio with every feature at its default. And on every host, the approve hook, asked about every tool the server lists under every name that host gives it, allows exactly the read-only ones and says nothing about the rest. #267 (B3) is already pinned for the two document tools the list misses, so while B3 is registered in `known_bugs.py` those two are left out, and any other difference fails.
- **Capture's log** (`test_adv_hook_capture_log.py`). Capture runs in the background on every host, so its log is the only account of what it did. On Claude Code, against the fakes, it logs a line when it skips a turn that only says to carry on, finds no typed prompt, finds no store and no login, and stores the fact a turn states.
- **Known bugs.** These tests found twelve bugs in the hooks, registered as B55 to B66 in `tests/harness/known_bugs.py`. Each is pinned as a strict expected failure in the module whose property it breaks. Each pin raises `Reproduced` only after it has seen its own bug's symptom, as "Known bugs and security findings" above describes, so any other failure in the same test fails the run.
  - `test_adv_hook_outcomes.py` pins three pairs of outcomes that cannot be told apart. A local store that cannot open is reported as "not configured" (B55, #337). On Codex, Copilot and OpenCode, "no matching memories" and "not configured" look identical, because recall shows nothing and logs nothing for either (B56, #338). Session start reports a hosted store it could not reach as "nothing stored yet" on Claude Code, and as no store at all on the other hosts (B57, #339).
  - `test_adv_hook_approve.py` pins B58 (#340). On Cursor and OpenCode the approve hook splits a tool name at a single "_", so a read-only tool whose name reaches it bare, or joined to the server name with one underscore, is never approved.
  - `test_adv_hook_client_config.py` pins B59 (#341). The hooks never find a store configured in Codex's, Cursor's or OpenCode's own MCP config.
  - `test_adv_hook_capture_log.py` pins B60 (#342). Capture logs nothing when a hook triggered the Stop, when the payload names no transcript, when the path names no file, and when the transcript has not grown since the last capture.
  - `test_adv_hook_dedup.py` pins B61 (#343). The first prompt of a session injects the standing preferences again, right after session start injected them.
  - `test_adv_hook_daemon.py` pins B62 (#344). A recall whose first read finds nothing starts two daemons. The pin waits up to 5 seconds for the second daemon to record its pid. While the bug is present that wait ends at once, because both pids are recorded before the first daemon answers. Once the bug is fixed, the test waits the whole 5 seconds on every run, so the fix should shorten the wait or move the test to the nightly tier.
  - `test_adv_hook_hostile.py` pins B64 (#346) and B65 (#347). A payload nested 100,000 levels deep crashes the body of every hook, and a prompt that holds half of a surrogate pair makes recall fail.
  - B63 (#345) and B66 (#348) take real time to show, so they are pinned only in the nightly tier, in `nightly/test_adv_hook_timeouts_nightly.py`. A hosted endpoint that never answers keeps session start and recall past their limits, and a 16 MB prompt keeps recall past its limit. While both bugs are present, their pins wait out about 70 seconds of limits between them.
  - The nightly tier also pins B55, B56, B57 and B64 on all five hosts, in `nightly/test_adv_hook_matrix_nightly.py`. The fast tier pins them on the two hosts its matrices cover.
- **Fixtures** (`conftest.py`). `hooks` builds runners for one test, each with a short home of its own, and closes them when the test ends; `store` is a store file that holds one memory; `store_env` names it to the hooks through `support.store_env(path)`, which names any other store the same way. `clis`, a fresh `FakeClis`, comes from `tests/adversarial/conftest.py`, because `test_adv_hooks.py` uses it too.
...
- **What the comparison sets aside.** Two stores that were told the same things still differ in ways that say nothing about the surface, and `compare.py` sets exactly those aside. It labels every id by what it names, because each store draws its own random ids; an id that no returned object names is numbered in the order it first appears. It replaces an instant a store took from its clock during the run with `<wall clock>`. An instant a surface returns as a `datetime` counts only if it falls inside the run, give or take a few milliseconds, so a surface that stamps a write even 35 seconds off fails; `test_a_hosted_instant_stamped_seconds_off_is_a_difference` plants that skew in the fake to show it. An instant in a tool's reply is written to the minute, rounding down, so there a minute either side of the run still counts. Two instants a store derives from its clock get markers of their own: a confirm token's expiry, ten minutes after its preview and kept in whole seconds, and the start of `memory_profile`'s default window, seven days before the call. Any other instant is compared as it is. A run of about eight minutes or longer is refused, because the windows for a clock reading and for a token's expiry would then overlap, and an instant in both could be replaced with either. It replaces a preview's confirm token, which is a signature over ids, sorts the claims a confirmed `forget_matching` closed, because they come back in the token's id order, and leaves out a write receipt's `latency_ms`. A claim's bookkeeping keys in `meta` are compared through the claim's own `salience_base` and `last_observed` instead, because the hosted API carries them differently. Two floats count as equal within 1e-6, because a search score drifts in the ninth decimal place between two runs a second apart, and two NaNs count as equal, while NaN against a number is a difference. Enums and instants keep their type, so a surface that returns a plain string in their place fails. Everything else is compared, and a failure lists every field that differs, with its path and both values. `test_adv_parity_compare.py` pairs each of these rules with a check that a real difference still shows.
- **The library's four clients.** `test_adv_parity_library.py` runs one program through the synchronous `Memvara`, `AsyncMemvara`, and the hosted clients `RemoteMemvara` and `AsyncRemoteMemvara`, each hosted client against a `FakeV1` of its own. The program writes facts with `remember` and `add`, reads them with `search`, `recall`, `get`, `history` and `why`, closes them with `delete`, `forget`, `forget_matching` with and without its confirm token, and the two ways of ending, and it covers the document methods, `stats`, `standing` and `profile`. Every step is its own test, which compares one client's answer with the synchronous library's. Where the hosted clients differ on purpose, a test of its own checks the difference and names where it is documented: a claim has the default in the four fields `/v1` does not carry, a search result's ranking has the default in the five fields the wire does not carry, `stats()` also carries the two join counts, `recall(budget=...)` and `recall(valid_at=...)` are refused, and the status of a document the caller cannot see is a `KeyError` on both sides, in each side's own words. memvara/memvara#298 tracks giving the hosted recall a time axis; when that lands, the refusal test fails and the dated recall joins the step-by-step comparison. The hosted clients also have `end()`, which the library does not, and a test checks that it leaves a hosted store holding what `delete(close="ended")` and `forget(close="ended")` leave a local one holding.
- **The MCP text.** `test_adv_parity_mcp.py` runs one session of tool calls through three surfaces: the in-process server, which is `MemvaraMCPServer.handle_line` built from the environment the way `memvara.server.cli.main` builds it; the stdio server in local mode; and the stdio server in cloud mode, pointed at `FakeV1` on 127.0.0.1. The in-process server and the local stdio server must write exactly the same text at every step, because only the transport separates them. Cloud mode must write the same text too, except where the code documents that it does not, and a test of its own checks each of those: `memory_stats` has no `storage:` line, because a hosted deployment has no local store to describe; `memory_standing` never says how many preferences it left out, because `GET /v1/standing` reports no total; `memory_recall` refuses `budget` and `valid_at`; and in cloud mode, the descriptions of `memory_forget` and `memory_end` promise that, given a predicate rather than a claim id, they close only the current values of that fact, because a hosted deployment on a release from before memvara/memvara#282 does not close a value stored to begin later. A reply that lacks an id a later step needs does not stop the session: the later step runs with an id no store holds, so each step's own comparison shows what differs. On the coverage checklist, this comparison covers every tool the session calls, and the three variables a cloud-mode server reads to reach the fake, `MEMVARA_MODE`, `MEMVARA_API_KEY` and `MEMVARA_SERVER_URL`, since the cloud replies would differ if any were ignored. `test_the_session_calls_every_tool_it_claims_to_cover` keeps the list of tools in its `covers` mark equal to the tools the session calls.
- **What it found.** Three differences that nothing documents, each filed and pinned as a strict expected failure. #334 (B52): a write receipt read through a hosted client leaves `accumulated`, `disputed`, `collapsed` and `retyped` empty where the local receipt reports what the write did, so in cloud mode `memory_remember` leaves out the notes built from them. That includes both kinds of re-filing: a memory type the caller asserted, and `procedural` refused for a subject other than the user. Both programs make the writes that fill those lists, and their comparison on the hosted surfaces is pinned: it raises `known_bugs.Reproduced` only when the emptied lists, or the missing notes, are the whole difference, so anything else that differs in the same step still fails the run. #335 (B53): a hosted receipt always reports 0 for `ungrounded` and `polluted`. #336 (B54): the section of docs/API.md on a hosted deployment does not name `end()`, `health()`, `whoami()`, `bind()` or `merge_predicate()`, though only one client has each. `test_adv_parity_known_bugs.py` shows each symptom on its own.
- **It catches what it is meant to catch.** It was run against ten deliberate faults, each in a scratch copy of the checkout, and every one failed the tests that should catch it: a hosted claim losing its `expire_reason`, `AsyncMemvara.profile` leaving out its search, the hosted `delete(close="ended")` retiring the claim instead of ending it, a hosted `why()` losing its source turns, the stdio server anchoring every read, `memory_stats` losing its `storage:` line on a local store, a hosted confirm token said to live two days longer, a hosted retirement stamped five hours early, a hosted receipt dropping the claims it added, and a hosted claim carrying its memory type as plain text.
...
The folder takes about 2 seconds on a laptop, which is within the fast tier's budget of about 6 seconds for this workstream.
