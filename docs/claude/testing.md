...
- **What the comparison sets aside.** Two stores that were told the same things still differ in ways that say nothing about the surface, and `compare.py` sets exactly those aside. It labels every id by what it names, because each store draws its own random ids; an id that no returned object names is numbered in the order it first appears. It replaces an instant a store took from its clock during the run with `<wall clock>`, allowing a minute either side because the tools write instants to the minute. Two instants a store derives from its clock get markers of their own: a confirm token's expiry, ten minutes after its preview, and the start of `memory_profile`'s default window, seven days before the call. Any other instant is compared as it is, so a surface whose instant is off by hours or days fails. It replaces a preview's confirm token, which is a signature over ids, sorts the claims a confirmed `forget_matching` closed, because they come back in the token's id order, and leaves out a write receipt's `latency_ms`. A claim's bookkeeping keys in `meta` are compared through the claim's own `salience_base` and `last_observed` instead, because the hosted API carries them differently. Two floats count as equal within 1e-6, because a search score drifts in the ninth decimal place between two runs a second apart. Enums and instants keep their type, so a surface that returns a plain string in their place fails. Everything else is compared, and a failure lists every field that differs, with its path and both values. `test_adv_parity_compare.py` pairs each of these rules with a check that a real difference still shows.
