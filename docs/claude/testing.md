...
- **A time limit can be shrunk instead of waited out.** `patches={"lib.extract.TIMEOUT_SEC": 1.0}` sets that module attribute inside the hook process before the hook runs, through a small launcher that binds the host first, as `run.py` does, and then calls `run.py`'s own `main`. A patch that names an attribute the hooks do not have is refused with `ValueError`, so a renamed limit cannot leave a test waiting out the real one. Patches cannot reach a capture that a host hands to a child, because `run.py` starts that child afresh, so they are refused there too.
...
- **Where the hooks find the store** (`test_adv_hook_client_config.py`). On every host whose client config is JSON, session start finds the store that the host's first client config file names, and a `MEMVARA_DB` in the hook's own environment wins over it. On Codex, the tests name the store through that environment, which works.
- **Hostile payloads** (`test_adv_hook_hostile.py`). Every hook gets `{}`, empty stdin, text that is not JSON, a JSON list, invalid UTF-8, a prompt that is a lone surrogate, fields of the wrong type, unknown fields, 8 MB, and nesting 100,000 levels deep, against a store that holds one memory. Each must exit 0, write nothing to stderr, print nothing or one JSON object, and finish within its host's limit. A payload that is not a JSON object once read must be answered exactly as `{}` is, with no crash of the hook's body logged by `run.py`, because `read_event` promises that anything unreadable becomes an empty event. No payload may start an extraction. Invalid UTF-8 is sent twice, once with `PYTHONIOENCODING=utf-8:strict` and once with `utf-8:surrogateescape`, because a child's stdin decodes it one of those two ways depending on the locale, and the result must not depend on the machine. The fast tier covers Claude Code and Cursor, which between them take every path through `run.py`; the nightly tier covers all five hosts and adds three JSON values that are not objects.
