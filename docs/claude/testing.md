...
- **The recall daemon is off unless a test asks for it.** `daemon=True` lets the recall hook start its background daemon, which `child_env` forbids because the daemon outlives the hook. `wait_for_daemon()` returns its socket and its pid once it accepts a connection. The pid comes from the kernel, through the socket (`socket_peer_pid`), because every daemon runs the same command. The runner also puts a small `sitecustomize` module first on the hook's `PYTHONPATH`, which Python runs when any process inheriting that path starts, so every daemon the hooks start records its pid: `daemon_pids()` lists them, which lets a test count the daemons one hook started. `close()` kills each recorded daemon that still runs this checkout's `daemon.py`, and each one listening under the runner's home, so a daemon that no socket path leads to any more is stopped too. The socket lives under the runner's home, and macOS refuses a unix socket path longer than 104 bytes, so a home for a daemon test needs a short path. Windows has no unix sockets, so these tests skip there.
- **A time limit can be shrunk instead of waited out.** `patches={"lib.extract.TIMEOUT_SEC": 1.0}` sets that module attribute inside the hook process before the hook runs, through a small launcher that binds the host first, as `run.py` does, and then calls `run.py`'s own `main`. A patch reaches code that reads the module attribute when it runs; a module that copied the value with `from x import NAME` keeps its own copy. A patch that names an attribute the hooks do not have is refused with `ValueError`, so a renamed limit cannot leave a test waiting out the real one. Patches cannot reach a capture that a host hands to a child, because `run.py` starts that child afresh, so they are refused there too.
...
- **Known bugs.** These tests found twelve bugs in the hooks, registered as B55 to B66 in `tests/harness/known_bugs.py`. Each is pinned as a strict expected failure in the module whose property it breaks. Each pin raises `Reproduced` only after it has seen its own bug's symptom, as "Known bugs and security findings" above describes, so any other failure in the same test fails the run.
  ...
  - `test_adv_hook_daemon.py` pins B62 (#344). A recall whose first read finds nothing starts two daemons.
...
- **Fixtures** (`conftest.py`). `hooks` builds runners for one test, each with a short home of its own, and closes them when the test ends; `store` is a store file that holds one memory; `store_env` names it to the hooks; `clis` is a fresh `FakeClis`.
