...
| The adversarial test suite: tiers, the harness, the skip ledger, known bugs, the reference model, crash tests, scripted scenarios, fakes, documentation checks, stores from old releases, protocol fuzzing, the coverage checklist | [testing.md](testing.md) | `tests/harness/`, `tests/adversarial/`, `tests/scenarios/` |
