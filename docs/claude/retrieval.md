...
- **A cosine threshold belongs to an embedding space.** The grounding rescue, the
  ...
  its own row there, measured with `bench/embedder_calibration.py`, or those three checks
  read its cosines with `BASELINE`'s 0.40 and 0.97, which were never measured in its space.
