...
switches it on; the names are `FEATURES` in `memvara/server/config.py`, the ones off by
default are `FEATURES_OFF_BY_DEFAULT` (only `extraction_chunks` today), and an unknown name
is refused.
