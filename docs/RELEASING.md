...
| `ci` | Calls `.github/workflows/ci.yml` **on the tagged commit**: 3.10–3.13 on Linux plus macOS and Windows, coverage gated at 100%, mypy, the no-extras import job, and the LOCOMO retrieval regression, which fetches the 2.8 MB dataset from `snap-research/locomo` on GitHub, so a release cannot be cut while that file is unreachable. It calls rather than restates, so there is one matrix in this repository and it cannot drift. A tag push starts nothing else, so without this job the release would be gated on whatever CI last happened to run. |
