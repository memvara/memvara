...
launched the process — never from tool input. `Engram.scope()` returns a `ScopedEngram`
...
INSTRUCTIONS = (
    "Engram is this user's long-term memory: structured facts with full history, stored "
...
class EngramMCPServer:
    """An `Engram` exposed as MCP tools over JSON-RPC.

    Owns the memory it is given: `close()` closes the underlying store, so whatever built
    the `Engram` does not have to keep a second reference alive to shut it down.
    """

    def __init__(self, memory: Engram, *, tenant: str | None = None,
