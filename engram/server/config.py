...
__all__ = ["ConfigError", "ServerConfig", "build_engram"]

_TRUE = {"1", "true", "yes", "on"}
_FALSE = {"0", "false", "no", "off", ""}

#: Backends selectable from the environment. Anything needing constructor arguments —
#: a custom model, an injected client — is a reason to import `EngramMCPServer` and wire
...
class ServerConfig:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> "ServerConfig":
        ...
        path = (env.get("ENGRAM_DB") or "").strip()
        if not path:
            raise ConfigError(
                "ENGRAM_DB is not set, so there is nowhere to remember anything. Point "
                "it at a file the client can write; it is created on first use:\n\n"
                f"{EXAMPLE_CONFIG}\n\n"
                "Use ENGRAM_DB=:memory: only for a smoke test — that store is discarded "
                "when this process exits.")

        backend = (env.get("ENGRAM_LLM") or "none").strip().lower()
        if backend not in _BACKENDS:
            raise ConfigError(
                f"ENGRAM_LLM={backend!r} is not a backend. Use "
        ...
        return cls(
            ...
            tenant=(env.get("ENGRAM_TENANT") or "default").strip() or "default",
            user=_optional(env.get("ENGRAM_USER")),
            agent=_optional(env.get("ENGRAM_AGENT")),
            session=_optional(env.get("ENGRAM_SESSION")),
            read_only=_flag(env.get("ENGRAM_READ_ONLY"), "ENGRAM_READ_ONLY"),
            llm=backend,
        )
...
def _optional(raw: str | None) -> str | None:
    ...
    They do not in general, but a client that writes `"ENGRAM_USER": ""` into its
...
def _anthropic() -> Any:
    ...
    try:
        ...
        raise ConfigError(
            f"ENGRAM_LLM=anthropic needs the anthropic SDK and a key: {exc}") from exc


def build_engram(config: ServerConfig) -> Engram:
    """Open the store this server speaks for.

    The scope goes to the constructor as well as to `Engram.scope()` later, because the
    ...
    return Engram(
        config.path,
        # Passed explicitly even when it is the default: `Engram()` warns about a missing
