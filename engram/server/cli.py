"""`python -m engram.server` — process startup, and the one place that touches stdio.
...
from .config import EXAMPLE_CONFIG, ConfigError, ServerConfig, build_engram
from .mcp import EngramMCPServer

__all__ = ["main"]

USAGE = f"""\
engram-mcp {__version__} — Engram memory as an MCP server over stdio.
...
client, not run interactively. Configured entirely by environment:
  ...
  ENGRAM_DB          required. Path to the SQLite file; created on first use.
  ...
  ENGRAM_AGENT       narrows further; unset is usually right.
  ENGRAM_SESSION     narrows further still. Memory written here is not visible to
                     other sessions, so leave it unset for durable facts.
  ENGRAM_LLM         'none' (default, offline, extracts only recognised sentence
                     forms) or 'anthropic' (needs ANTHROPIC_API_KEY).
  ENGRAM_READ_ONLY   '1' to hide every tool that writes.
...
def main(argv: Sequence[str] | None = None, *, env: Mapping[str, str] | None = None,
    ...
    if args:
        ...
        if args in (["--help"], ["-h"]):
            ...
            return 0
        print(f"engram-mcp: unexpected argument {args[0]!r}\n\n{USAGE}", file=err)
        return 2
    ...
    try:
        ...
        print(f"engram-mcp: {exc}", file=err)
        return 2

    server = EngramMCPServer(memory, read_only=config.read_only, **config.scope_kwargs)
    try:
