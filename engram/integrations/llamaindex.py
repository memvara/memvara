"""LlamaIndex: a long-term memory block and a retriever, mapped onto engram.
...
**Which LlamaIndex abstraction is engram?** There are three candidates and only one of
them is honest.
...
``BaseMemoryBlock`` — yes, this one
    ...
    That is engram's `recall()` and `add()` with the arguments already in the right
    order. It is also the only one of the three where the *write path* is engram's: the
...
``BaseMemory`` — no. It is a chat buffer.
    `get`/`put`/`set`/`get_all`/`reset` over `ChatMessage`, i.e. the transcript. Engram
    ...
    composable `Memory` with an `EngramMemoryBlock` in `memory_blocks=` — that is the
...
``BasePydanticVectorStore`` — no, and this is the interesting refusal.
    ...
    `add(nodes)` receives nodes carrying vectors from that same model. Engram's index
    ...
    never arrives at that interface — or store them, at which point engram is a numpy
    matmul with extra steps and every reason to use it is gone. `as_vector_store()`
    raises and names `EngramRetriever`, which gets the query as a string and keeps all
    of it.

What is lost through `BaseMemoryBlock`: `_aget` returns **a string**, so scores,
provenance and `as_of` have nowhere to go. `EngramMemoryBlock.search()` and `.history()`
are the escape hatch, and `EngramRetriever` is the same retrieval with the structure
intact.
"""
...
_NO_VECTOR_STORE = (
    "Engram is not a vector store, and standing in for one would throw away the reason "
    "to use it.\n\n"
    ...
    "add(nodes) is handed nodes already carrying vectors from that model. Engram's "
    "index belongs to its own embedder (see engram.EmbedderMismatchError), and its "
    ...
    "Use engram.integrations.llamaindex.EngramRetriever instead: it implements "
...
_NO_CHAT_MEMORY = (
    ...
    "ChatMessage — and engram's unit of memory is a reconciled bitemporal claim, not a "
    "message. Standing in for the buffer would mean either serving a transcript engram "
    "does not maintain, or serving facts dressed as messages so the agent cannot tell "
    "what was said from what is believed.\n\n"
    "The supported shape is the composable one, where engram is long-term memory beside "
    "the buffer rather than instead of it:\n"
    "    from llama_index.core.memory import Memory\n"
    "    from engram.integrations.llamaindex import EngramMemoryBlock\n"
    "    memory = Memory.from_defaults(\n"
    "        session_id='s1',\n"
    "        memory_blocks=[EngramMemoryBlock(memory=mem, user='alice')],\n"
    "    )"
)


class LlamaIndexCompatError(IntegrationError):
    """A LlamaIndex call with no honest translation onto engram."""
...
def _role_of(message: Any) -> str:
    """Engram's role for a `ChatMessage`.

    `MessageRole` is a `str` enum whose values are already engram's vocabulary
...
class _MemoryBlock:
    """`EngramMemoryBlock`'s behaviour, with no LlamaIndex in sight.
    ...
    `import engram`. It defines no `__init__` — `BaseMemoryBlock` is a pydantic model
    and owns construction; the fields are declared on the composed class.

    The two the framework calls (`_aget`, `_aput`) cross to engram through
    ...
    event loop for exactly as long as the write takes — see `engram.aio`, which exists
    ...
    def _engram(self) -> tuple[Any, Any]:
    ...
    async def _aget(self, messages: Sequence[Any] | None = None,
        ...
        """The block's contribution to the prompt: engram's `recall()` for this turn.
    ...
    async def _aput(self, messages: Sequence[Any]) -> None:
        """Absorb messages through engram's write path.
    ...
    def search(self, query: str, **kw: Any) -> list[Any]:
        """`Engram.search` at this block's scope — scores, provenance and `as_of=`.
    ...
    def history(self, subject: str, predicate: str) -> list[Any]:
        """`Engram.history` at this block's scope: every value the slot ever held."""
        memory, scope = self._engram()
...
class _Retriever:
    """`EngramRetriever`'s behaviour, with no LlamaIndex in sight.
    ...
    def _retrieve(self, query_bundle: Any) -> list[Any]:
        ...
        the query with its own model, that vector is not in engram's space, and using it
        ...
        return [
            node_with_score(
                node=text_node(text=result.text, id_=metadata["engram_id"],
                               metadata=metadata),
                score=result.score,
            )
...
def as_vector_store(memory: Any) -> Any:
    """Always raises. Engram is not a vector store — see `_NO_VECTOR_STORE`.
    ...
    `EngramRetriever` is better than finding nothing and writing a bad one.
    """
...
def _block_class(base: type) -> type:
    ...
    in the prompt template, and making every caller type `name="engram"` is friction
    with no decision behind it.
    """

    class EngramMemoryBlock(_MemoryBlock, base):  # type: ignore[misc, valid-type]
        """Engram as a LlamaIndex long-term memory block.

            Memory.from_defaults(session_id="s1",
                                 memory_blocks=[EngramMemoryBlock(memory=mem,
                                                                  user="alice")])

        `_aget` contributes `Engram.recall()` for the current turn; `_aput` runs the
        batch through engram's write path. Scores, provenance and time travel do not fit
...
def _retriever_class(base: type) -> type:
    ...
    class EngramRetriever(_Retriever, base):  # type: ignore[misc, valid-type]
        """Engram as a LlamaIndex retriever.

            index_or_engine.as_query_engine(retriever=EngramRetriever(mem, user="alice"))
...
def __getattr__(name: str) -> Any:
    # PEP 562, same as `engram.llm`: naming a class here must not import
    # llama-index-core, or the numpy-only install stops being able to `import engram`.
    if name == "EngramMemoryBlock":
        (base,) = require(f"{_PKG}.memory", "BaseMemoryBlock", extra="llama-index",
                          needs=_NEEDS)
        return _block_class(base)
    if name == "EngramRetriever":
...
__all__ = [
    "EngramMemoryBlock", "EngramRetriever", "LlamaIndexCompatError",
