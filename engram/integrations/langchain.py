"""LangChain: chat history and retrieval, mapped onto engram.
...
``EngramRetriever`` — a clean fit
    `BaseRetriever` asks for "given a query, return documents". That is exactly what
    `Engram.search` is, and everything engram knows survives the crossing: hybrid
...
``EngramChatMessageHistory`` — a lossy fit, deliberately not disguised
    `BaseChatMessageHistory` models memory as **a list of messages**, and engram's unit
    ...
    looks: the memory engram built is not in it, and engram's tier-0 hash dedupe means
    ...
    are the escape hatch, and `EngramRetriever` is the supported way to put memory in a
    prompt.

    `clear()` is the sharper edge and it **raises** by default. LangChain documents it
    as "remove all messages from the store"; engram's two candidate meanings are
...
#: LangChain message `type` -> engram `Episode.role`. `chat` is absent on purpose: a
...
#: through `Engram.add` or the MCP server still belongs in the transcript.
...
_NO_CLEAR = (
    ...
    "engram has two different operations that could be meant — so it refuses to pick.\n\n"
    ...
    "There is deliberately no third option that retires: engram can retire one claim "
...
_NO_TRANSCRIPT_PROVENANCE = (
    "EngramChatMessageHistory.messages returns the stored turns, which is what "
    ...
    "of engram's own structure is in what you just read. To reach it: "
    "history.recall(query) for a prompt-ready block, history.search(query) for scored "
    "results carrying provenance, or EngramRetriever for the same thing as a LangChain "
    "Runnable.\n\n"
    "It is also not a verbatim log. Engram's first write tier deduplicates turns by "
...
class LangChainCompatError(IntegrationError):
    """A LangChain call with no honest translation onto engram."""


class EngramTranscriptWarning(UserWarning):
    """`messages` was read, and a transcript is not what engram stores.
...
def _role_of(message: Any) -> str:
    """Engram's role for a LangChain message.
...
class _ChatHistory:
    """Everything `EngramChatMessageHistory` does, with no LangChain in sight.

    A mixin rather than a subclass because `BaseChatMessageHistory` cannot be imported
    at module scope without making langchain-core a hard dependency of `import engram`.
    ...
    def __init__(self, memory: Any, *, tenant: str | None = None, user: str | None = None,
        ...
        if on_clear not in _ON_CLEAR:
            raise ValueError(
                ...
                "engram.integrations.langchain.EngramChatMessageHistory for what each "
                "one does"
            )
    ...
    def _stored_turns(self) -> list[Episode]:
        ...
        Engram has no scoped, ordered episode enumeration to call instead; that gap is
    ...
    def messages(self) -> list[Any]:
        ...
        Lossy in two directions, both named by `EngramTranscriptWarning`. The claims
        engram extracted, what they superseded and where they came from are not
        representable in a `list[BaseMessage]` and are not in here — `recall`/`search`
        reach them. And engram's tier-0 hash dedupe means an exactly repeated turn was
    ...
    def _warn_transcript(self) -> None:
        ...
        warnings.warn(_NO_TRANSCRIPT_PROVENANCE, EngramTranscriptWarning, stacklevel=3)

    def recall(self, query: str, **kw: Any) -> str:
        """`Engram.recall` at this scope — the prompt-ready block `messages` cannot be.
    ...
    def search(self, query: str, **kw: Any) -> list[Any]:
        """`Engram.search` at this scope, including `as_of=` and `include_episodes=`."""
        return self.memory.search(query, **self._kw, **kw)

    # -- writing -------------------------------------------------------------

    def add_messages(self, messages: Sequence[Any]) -> None:
        """Ingest a batch of messages through engram's write path.
    ...
    def __repr__(self) -> str:
        return (f"<EngramChatMessageHistory {self.scope.key()} limit={self.limit} "
...
def _to_message(turn: Episode, classes: dict[str, Any]) -> Any:
    ...
    blocks. Engram stores the text of a turn; a turn whose meaning is in its structure
...
class _Retriever:
    """`EngramRetriever`'s behaviour, with no LangChain in sight.
...
def _history_class(base: type) -> type:
    ...
    class EngramChatMessageHistory(_ChatHistory, base):  # type: ignore[misc, valid-type]
        """Engram as a LangChain chat message history, bound to one scope.

            EngramChatMessageHistory(mem, user="alice", session="s1")
        ...
        `on_clear` says which of engram's two deletions you meant.
        """
...
def _retriever_class(base: type) -> type:
    ...
    `arbitrary_types_allowed` is what lets `memory` hold an `Engram`; `Any` alone would
    ...
    class EngramRetriever(_Retriever, base):  # type: ignore[misc, valid-type]
        """Engram as a LangChain retriever.

            EngramRetriever(memory=mem, user="alice", k=5)
...
def __getattr__(name: str) -> Any:
    # PEP 562, same as `engram.llm`: naming a class here must not import langchain-core,
    # or the numpy-only install stops being able to `import engram`.
    if name == "EngramChatMessageHistory":
...
__all__ = [
    "EngramChatMessageHistory", "EngramRetriever", "LangChainCompatError",
    "EngramTranscriptWarning",
]
