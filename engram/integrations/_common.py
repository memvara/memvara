...
become a hard dependency.** `import engram` has to keep working with numpy alone, and CI
...
class IntegrationError(NotImplementedError):
    """A framework call with no honest translation onto engram.
...
def require(module: str, *names: str, extra: str, needs: str) -> tuple[Any, ...]:
    ...
    try:
        ...
        raise ImportError(
            f"this adapter needs {module}: pip install 'engram[{extra}]' "
            f"(or {needs} directly). Engram itself needs numpy and nothing else; the "
...
def bind(memory: Any, *, tenant: str | None = None, user: str | None = None,
         agent: str | None = None, session: str | None = None) -> tuple[Engram, Scope]:
    """Resolve `(engram, scope)` from an `Engram` or a `ScopedEngram`, plus overrides.

    Adapters keep both halves rather than a `ScopedEngram`, because two of them need the
    ...
    A `ScopedEngram` is accepted because a server layer holds one per request and has no
    public way to get the underlying `Engram` back out; reaching for `_mem` here is the
    price of that, and is noted in the workstream report as a missing accessor rather
    than papered over.
    """
    inner = getattr(memory, "_mem", None)
    if inner is not None:                     # a ScopedEngram: start from its scope
...
def scope_kw(scope: Scope) -> dict[str, Any]:
    """A `Scope` as the four keyword arguments every `Engram` method takes."""
    return {"tenant": scope.tenant, "user": scope.user, "agent": scope.agent,
            "session": scope.session}


def result_metadata(result: Any) -> dict[str, Any]:
    """Everything engram knows about one search result, as a flat dict.
    ...
    if isinstance(result, EpisodeResult):
        ...
        return {**common, "engram_id": turn.id, "role": turn.role,
