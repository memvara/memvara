...
The migration problem is not "can engram hold these strings" — it is that nobody accepts
...
import exists to reconstruct. Use `Engram.purge()` if a scope must genuinely go.
...
def _payload_scope(payload: Mapping[str, Any], fallback: Scope) -> Scope:
    """mem0's flat entity triple as an engram scope, defaulting to the import's."""
    return Scope(
        fallback.tenant,
...
def _triple(mem: Engram, item: Mapping[str, Any], chunk: Sequence[Episode],
...
def _extract(mem: Engram, sources: Sequence[Episode], llm: LLM, batch_size: int,
...
def _contested(mem: Engram, scopes: Sequence[Scope], note_predicate: str) -> list[ContestedSlot]:
