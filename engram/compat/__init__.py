"""Running an existing mem0 integration on engram, and bringing its data along.

Two pieces, and the second is the one that decides whether a migration happens at all:

* `engram.compat.mem0.Memory` — mem0 2.x's method surface, mapped onto `Engram`. An
...
* `engram.compat.mem0_import.import_mem0` — an importer that reads mem0's own mutation
...
Everything here is built on engram's public API and adds no dependency on mem0 — the
