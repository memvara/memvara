...
def default_embedder(dim: int = 512) -> Embedder:
    ...
    Note what this means for an *existing* store: installing `engram[local-embed]`
    changes what this returns, and the new vectors are incomparable with the old ones.
    That is why `Engram()` checks the embedder against the store it opens instead of
    discovering the swap on the first read — see `engram/embed/fingerprint.py` and
    `Engram.reembed()`.
    """
